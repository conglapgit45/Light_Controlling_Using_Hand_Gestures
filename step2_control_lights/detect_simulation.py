...
class LightGesture:
    ...
    def light_simulation(self, img, light):
        # Append a white rectangle at the bottom of the image
        height, width, _ = img.shape
        rect_height = int(0.15 * height)
        rect_width = width
        white_rect = np.ones((rect_height, rect_width, 3), dtype=np.uint8) * 255
        img[-rect_height:, :] = white_rect

        centers = [
            (int(rect_width * 0.25), height - int(rect_height / 2)),
            (int(rect_width * 0.5), height - int(rect_height / 2)),
            (int(rect_width * 0.75), height - int(rect_height / 2))
        ]
        radius = 16
        colors = [(0, 255, 0) if light[0] else (0, 0, 0), (0, 255, 0) if light[1] else (0, 0, 0), (0, 255, 0) if light[2] else (0, 0, 0)]
...
if __name__ == "__main__":
    model_path = "./models/model_02-12 15_46_NeuralNetwork_best"
